...
| 2022 | 2 | 24 | 题名、作者、摘要与页码已按第二期完整 PDF 核对 | 未开放 |
| 2023 | 3 | 22 | 来自已提供的扫描目录；3 条摘要为空 | 未开放 |
| 2024 | 4 | 24 | 来自已提供的扫描目录 | 未开放 |
| 2025 | 5 | 24 | 来自已确认的 2025 目录 | 未开放 |
| 2026 | 6 | 30 | Beta-B-2 PDF 目录；页码暂填 1 | 未开放 |

网站合计生成 147 条“年度文章记录”。这里的“文章”是目录题名、作者、学科、摘要和页码，不代表网站已经提供文章全文。

### 2.2 发布前仍需人工确认的事项

代码中保留了明确的待核信息，主要包括：

- 2026 年目录、封面焦点、制作团队及后记已按 Beta-B-2 更新；目录页码全部暂填 1，待最终排版确认；
- 历年封面、摄影、标志、二维码及部分素材缺少独立的网页发布许可记录；
- 2021—2023 的部分编辑署名仍需编辑确认；
...
| `/questions-answers/` | `src/pages/questions-answers/index.astro` | 147 条年度文章的浏览器端关键词、学科、年份/期次组合筛选 | `articles`、`contentDisciplines`、`issues` |
| `/disciplines/` | `src/pages/disciplines/index.astro` | 七个现行板块和“其他”入口；链接视觉规范 | 学科组件内部读取数据 |
| `/disciplines/:slug/` | `src/pages/disciplines/[slug].astro` | 单一现行学科的跨年文章聚合、统计和前后板块导航 | `disciplines`、`articles` |
| `/disciplines/other/` | `src/pages/disciplines/other/index.astro` | 脑神经科学、电子两个历史板块的颜色档案、文章聚合和前后板块导航 | `historicalDisciplines`、`articles` |
...
首页刊名、2026、总第 6 期仍为页面文字；文章数量已由本期目录自动计算。

**本期页 `/2026/`**

1. `annual-hero`：本期刊名、口号、期号、订阅入口和快捷链接；学科与文章数量从数据计算，总期号仍为页面文字；
2. `cover-focus`：显示 `coverFocusArticleSlugs` 命中的全部文章，并链接到年度目录锚点；
3. `cover-story`：显示 `siteMeta.coverStory` 的标题和封面介绍文字；
4. `SubscriptionPanel`：QQ 空间二维码和“更多订阅方式”占位。

`siteMeta.editorialNote` 目前没有在本期页或其他页面渲染。后记已依据 Beta-B-2 更新为陈逸轩署名、2026.9；若要展示，需新增页面区块。

**文章检索 `/questions-answers/`**
...
| `articles.ts` | 2021—2025 目录母版、2021—2026 年度目录映射与记录生成逻辑 |
...
| `catalog2026.ts` | Beta-B-2 的 30 篇目录，页码暂填 1 |
...
`disciplines` 数组位置会决定卡片顺序和前后板块导航；数据中的 `order` 决定详情 Hero 显示的编号。只改 `order` 不会重排页面。

### 7.7 年度目录匹配

`annualCatalogs` 登记 2021—2026 各期目录。2026 数据位于 `src/data/catalog2026.ts`，依据 Beta-B-2 PDF 的 30 篇目录录入，页码统一暂填 1。

`catalogForYear()` 对未登记年份直接抛错。增加任何年份时，必须同时登记真实年度目录，避免误用其他年份内容。
...
关闭 JavaScript 时全部文章仍可阅读，但不能筛选。当前 147 行适合浏览器线性过滤；数据规模明显增长后，应评估分页、静态索引或 URL 状态。
...
2026 年目录已独立录入 `src/data/catalog2026.ts`，来源为《非正式科学》2026 年刊 Beta-B-2 PDF。题名、作者、摘要和学科已按目录核对；**30 篇的 `page` 均暂填 1，不得按 Beta PDF 正文页码擅自改动**。

收到最终页码后，逐项修改该文件的 `page`，更新来源与页面中的页码说明，并核对年度页、学科页、文章检索和首页展示。文章数量自动计算；修改题名时尽量保留现有 slug，并核对 `siteMeta.coverFocusArticleSlugs` 对应的七篇文章。
...
- 确认没有把未授权或待审核内容误公开；
- 确认新增文章出现在正确年度和学科页；
- 确认 2026 使用独立目录，页码确定前保留暂填 1 的说明；
- 检查 about 中实际展示的人员范围；
- 按所用 PDF 版本检查封面故事、团队和后记。
...
1. 最终排版确认后更新 2026 目录页码，目前统一暂填 1。
2. 新版 PDF 发布后复核封面故事与目录变动。
3. 最终版确认后复核制作团队与后记；当前已按 Beta-B-2 同步。
...
1. 当前年份、六期、七板块和 2021—2026 范围大量硬编码。
