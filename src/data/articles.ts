...
type DirectoryArticle = Pick<
  Article,
  "slug" | "title" | "author" | "disciplineSlug" | "summary" | "page"
>;

const directoryReviewNotes = [
  "当前书籍工程的对应学科正文仍是占位内容，网站首版只可发布目录导读。",
  "页码来自 TOC.tex 的人工录入，需在正式 PDF 完成后复核。",
] as const;

const directoryArticle = (entry: DirectoryArticle): Article => ({
  ...entry,
  ...
  reviewNotes: directoryReviewNotes,
  source: {
    repository: "Informal-Science-2026",
    path: "Resources/TOC/TOC.tex",
    locator: "tocitem: " + entry.title,
  },
});
